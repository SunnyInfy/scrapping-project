...
app.get('/api/status/:motive', (req, res) => {
    const { motive } = req.params;
    res.json(activeScrapes[motive] || { status: 'idle', count: 0 });
});
// Trigger a scrape
app.post('/api/scrape/:motive/:source', async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            await scraper.close();
            if (results.length > 0) {
                await saveScrapedData(motive, source, results);
                clearCacheByPrefix(`results_${motive}`);
...
app.get('/api/results/:motive', async (req, res) => {
    const { motive } = req.params;
    ...
    try {
        ...
        const data = await getScrapedData(motive);
...
app.get('/api/export/:motive', async (req, res) => {
    const { motive } = req.params;
    if (!['rooms', 'jobs', 'deals'].includes(motive)) {
        return res.status(400).json({ error: 'Invalid motive.' });
    }
    try {
        ...
        if (!data) {
            data = await getScrapedData(motive);
        }
        ...
        const csv = Papa.unparse({
            ...
            data: orderedData.map((item) => columnOrder.map((col) => item[col] || ''))
        });
